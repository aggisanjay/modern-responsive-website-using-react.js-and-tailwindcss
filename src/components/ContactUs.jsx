...
const ContactUs = () => {
  ...
  const onSubmit=async(event)=>{
    ...
    if (data.success) {
      toast.success("Thank you for your sumission!");
      event.target.reset();
    } else {
  ...
  return (
    ...
    id='contact-us' className='flex flex-col items-center gap-7 px-4 sm:px-12 lg:px-24 xl:px-40 pt-30 text-gray-700 dark:text-white'>
      ...
      onSubmit={onSubmit} className='grid sm:grid-cols-2 gap-3 sm:gap-5 max-w-2xl w-full'>
        ...
        <button type='submit' className='w-max flex gap-2 bg-primary text-white text-sm px-10 py-3 rounded-full cursor-pointer hover:scale-103 transition-all' required>
